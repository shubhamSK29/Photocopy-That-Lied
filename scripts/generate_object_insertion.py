"""Generate object insertion variants for Dataset V2 with split safety."""
...
def main():
    """Generate object insertion variants with split safety."""
    print("=== Generating Object Insertion Variants with Split Safety ===")
    ...
    for split, split_sources in sources_by_split.items():
        if len(split_sources) >= 2:  # Need at least 2 sources for donor-target pairing
            # Select min(config.pilot_insertion_count, available) pairs per split
            available_pairs = len(split_sources) // 2
            pairs_needed = min(config.pilot_insertion_count // 3, available_pairs)  # Distribute across splits
    ...
    # Generate object insertion variants
    for i, (target, donor, split) in enumerate(selected_pairs):
        try:
            ...
            # Skip if destination region is too small
            if dest_w < 10 or dest_h < 10:
                print(f"Skipping {target['image_id']}: destination region too small ({dest_w}x{dest_h})")
                continue
            
            # Extract donor region (center of donor)
            donor_w = max(1, min(dest_w, w_donor))
            donor_h = max(1, min(dest_h, h_donor))
            ...
            try:
                ...
                print(f"Error in object_insertion for {target['image_id']}: {e}")
                continue
            ...
            if variant_id not in existing_image_ids:
                ...
                print(f"Generated object insertion: {variant_id} (donor={donor['source_id']}, split={split})")

        except Exception as e:
            print(f"Error generating object insertion for {target['image_id']}: {e}")
    
    print(f"\nGenerated {len(all_records)} object insertion variants")
    ...
    print("\n=== Object Insertion Generation Complete ===")
